...
MySQL/MariaDB and SQLite are the supported SQL dialects. Install the corresponding PDO driver. Exception mode is enforced; query failures must not look like empty successful results. `query()` returns associative rows (`[]` for a statement without rows), and `getVar()` returns the first value or null when no row exists.
...
Read filters use this formatter. Insert/update/delete use native PDO prepared statements and typed bound values. Raw SQL passed to `query()` is trusted application code. For custom prepared queries, use `getPdo()->prepare()` yourself. Never concatenate untrusted SQL or identifiers.
