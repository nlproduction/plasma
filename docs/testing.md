...
PHP 7.4 runtime compatibility is checked with PHP lint over `src/` and `php tests/runtime.php`. Consumers can install the runtime library on PHP 7.4; developing the repository itself requires PHP 8.1+ because Composer also resolves its Pest development dependencies.
